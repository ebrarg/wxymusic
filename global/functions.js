...
module.exports = (bot, utils, ytdl, config) => {
    ...
    fs.readdir("./commands/", (err, files) => {
        ...
        if (jsfiles.length <= 0) return console.log("Yuklenecek komut yok...");

        console.log(`Loading ${jsfiles.length} komut...`);
        jsfiles.forEach((f, i) => {
            let props = require(`../commands/${f}`);
            console.log(`${i + 1}: ${f} yuklendi!`);
    ...
    bot.loadCommand = (commandName) => {
        try {
            ...
            return utils.cmd_fail(`Error: ${err}\nCommand \`${commandName}\` komudu bulunamadi.`, `${config.prefix}reload <command>`);
        }
    };

    bot.unloadCommand = async (commandName) => {
        try {
            if (!commandName) return `\`${commandName}\` komudu buralarda yok! Tekrar dene!`;

            if (commandName.shutdown) await commandName.shutdown(bot);
            delete require.cache[require.resolve(`../commands/${commandName}.js`)];
            return false;
        } catch (err) {
            return utils.cmd_fail(`Error: ${err}\nCommand \`${commandName}\` bulunamadi.`, `${config.prefix}reload <command>`);
        }
    };
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            try {
                ...
                console.error(`Kanala katilamiyorum: ${err}`);
            }
        } else {
